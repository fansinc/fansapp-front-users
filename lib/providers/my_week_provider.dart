        ...
        myWeekMapData.forEach((key, value) {
          if (key == 'data') {
            ...
            for (int i = 0; i < value.length; i++) {
              final assetListIP = value[i]['Assets']['data'];
              List<String> assetList = [];
              List<String> assetType = [];
              for (int j = 0; j < assetListIP.length; j++) {
                assetList.add(value[i]['Assets']['data'][j]['links']['full']);
                assetType.add(value[i]['Assets']['data'][j]['type']);
              }
              myWeekModelList.add(
                MyWeekModel(
                  id: value[i]['id'],
