...
class NewsProvider with ChangeNotifier {
  ...
  Future<List<NewsModel>> fetchSetNews() async {
    ...
    try {
      ...
      const url = '${AppConfig.baseUrl}${AppConfig.newsUrl}';
            ...
            for (int i = 0; i < value.length; i++) {
              newsModelList.add(
                NewsModel(
                  ...
                  title: value[i]['title'],
                  assetType: value[i]['Assets']['data'][0]['type'],
                  assetUrl: value[i]['Assets']['data'][0]['type'] == 'image'
                      ? value[i]['Assets']['data'][0]['links']['full']
                      : value[i]['Assets']['data'][0]['linksvideo']['full'],
                  //['data'][0]['links']['full'],
                ),
              );
            }
